...
func newBridgeNetworkConfigurator(log hclog.Logger, alloc *structs.Allocation, bridgeName, ipRange string, hairpinMode bool, cniPath string, ignorePortMappingHostIP bool, nodeAttrs map[string]string) (*bridgeNetworkConfigurator, error) {
	...
	for _, svc := range tg.Services {
		if svc.Connect.HasTransparentProxy() {
			...
			log.Info("CNI debugging", "netCfg", string(netCfg)) // TODO(tgross): remove
			break
		}
	}
	if netCfg == nil {
	...
	c, err := newCNINetworkConfiguratorWithConf(log, cniPath, bridgeNetworkAllocIfPrefix, ignorePortMappingHostIP, netCfg, nodeAttrs)
	if err != nil {
		return nil, err
	}
	b.cni = c
